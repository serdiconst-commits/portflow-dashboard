...
export const getEirCategoryFromSubType = (
  ...
  if (normalized === "RI") return "IN EIR";
  if (normalized === "RO" || normalized === "DM") return "OUT EIR";
  return "";
};
...
export const getShipmentStatusFromTransaction = (
  ...
  if (subType === "RO" || subType === "DM" || stageId.includes("outgate")) {
    return "Picked Up";
  }
  if (subType === "RI" || stageId.includes("ingate")) {
    return "At Port";
  }
  return "Dispatched";
};
